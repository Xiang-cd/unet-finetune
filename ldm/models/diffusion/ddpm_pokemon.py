...
from ldm.modules.attention import CrossAttention, AdapterForward
...
class LatentDiffusion(DDPM):
    ...
    @torch.no_grad()
    def log_samples(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., unconditional_guidance_scale=1., unconditional_guidance_label=None,
                   use_ema_scope=True,
                   **kwargs):
    ...
    def configure_optimizers(self):
        ...
        if self.train_level is not None and "embeder" in self.train_level:
            raise ValueError("pokemon finetune does not tune unet")
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        if self.train_level is not None:
            ...
            if hasattr(self, "model_ema"):
                ...
                if "unet" in self.train_level:
                    torch.save(self.model_ema.state_dict(), os.path.join(root_path, f"{self.global_step}-unet-ema.pt"))
                elif "adapter" in self.train_level:
                    save_dir = os.path.join(root_path, f"{self.global_step}-ema")
                    os.makedirs(save_dir, exist_ok=True)
                    for name in dir(self.model_ema):
                        if "adapter" in name:
                            print(f"saving {name}")
                            torch.save(getattr(self.model_ema, name), os.path.join(save_dir, f"{name}.pt"))
                if "embeder" in self.train_level:
                    raise ValueError("make sure ema train embeder")
