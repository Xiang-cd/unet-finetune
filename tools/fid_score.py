...
def calculate_fid_given_train_and_paths(path_root, paths, device=None, batch_size=50, dims=2048, num_workers=8):
    """Calculates the FID of two paths"""
    if device is None:
        device = torch.device('cuda' if (torch.cuda.is_available()) else 'cpu')
    else:
    ...
    m_root, s_root = compute_statistics_of_path(path_root, model, batch_size,
                                        dims, device, num_workers)
    ms_list = []
    for path in paths:
        m2, s2 = compute_statistics_of_path(path, model, batch_size,
                                        dims, device, num_workers)
        ms_list.append((m2, s2))
    fid_list = []
    for obj in ms_list:
        m2 = obj[0]
        s2 = obj[1]
        fid_value = calculate_frechet_distance(m_root, s_root, m2, s2)
